function loadMakanan() {
    if (localStorage.list_data && localStorage.id_data) {
        ...
        if (list_data.length > 0) {
            ...
            data_app = "Makanan masih kosong ...";
        }


        $('#list-makanan').html(data_app);
        $('#list-makanan').hide();
        $('#list-makanan').fadeIn(100);
    }
}
    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        for (i in list_data) {
            if (list_data[i].id_data == id) {
                ...
                $("#emakanan").val(list_data[i].makanan);
    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        for (i in list_data) {
            if (list_data[i].id_data == id) {
                ...
                $("#lmakanan").val(list_data[i].makanan);
                list_data.splice(idx_data, 1);
            }
            idx_data++;
        }
...
function simpanData() {
    ...
    if (!liff.isInClient()) {
        ...
        liff.sendMessages([{
            'type': 'text',
            'text': "Pesanan Makanan baru berhasil disimpan"
        }]).then(function() {
            alert('Makanan Tersimpan');
...
function simpanEditData() {
    ...
    if (!liff.isInClient()) {
        ...
        liff.sendMessages([{
            'type': 'text',
            'text': "Pesanan Makanan yang diedit sudah tersimpan"
        }]).then(function() {
            alert('Makanan tersimpan');
        }).catch(function(error) {
            alert('Error ...');
...
function hapusData(id) {
    ...
    if (!liff.isInClient()) {
        ...
        liff.sendMessages([{
            'type': 'text',
            'text': "Pesanan Makanan sudah terhapus"
        }]).then(function() {
            alert('Makanan sudah dihapus');
        }).catch(function(error) {
            alert('Error');
        });
    }
...
function gantiMenu(menu) {
    ...
    else if (menu == "tambah-makanan") {
        $('#tambah-makanan').fadeIn();
        $('#list-makanan').hide();
        $('#edit-data').hide();
        $('#lihat-data').hide();
